...
const output =
  outputArgumentIndex >= 0
    ...
    : NodePath.join(desktopRoot, ".native", "ditto-telegram-ax");
...
const source = NodePath.join(desktopRoot, "native", "telegram-accessibility", "main.swift");
...
if (NodeFS.existsSync(output)) {
  ...
  if (NodeFS.statSync(output).mtimeMs >= newestInputModifiedAt) process.exit(0);
}
