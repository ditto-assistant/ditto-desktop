...
const sourceDirectory = NodePath.join(repoRoot, "native", "discord-sidecar");
...
function build(targetArch, destination) {
  const result = NodeChildProcess.spawnSync(
    "go",
    ["build", "-mod=readonly", "-trimpath", "-ldflags=-s -w", "-o", destination, "."],
    {
      cwd: sourceDirectory,
      encoding: "utf8",
      env: {
...
if (arch === "universal") {
  const temporaryDirectory = NodeFS.mkdtempSync(
    NodePath.join(NodeOS.tmpdir(), "ditto-discord-sidecar-"),
  );
  try {
    ...
    if (lipo.status !== 0) {
      throw new Error(lipo.stderr || `lipo exited ${String(lipo.status)}`);
    }
  } finally {
